...
`augmentations/` in this repository is the DocCreator source tree — see the
note at the end.
...
```python
from augment import apply_chain
...
That directory currently holds only DocCreator's build scaffolding — CMake
modules, bundlers, CI configs, AUTHORS. **The source is not there**: no `.cpp`
or `.hpp` file, and `CMakeLists.txt` calls `ADD_SUBDIRECTORY(${FRAMEWORK_PATH})`
and `${SOFTWARE_PATH}`, neither of which was committed. This port was written
against the upstream repository instead. If you want the C++ as a reference,
add it as a submodule rather than a copy:

```bash
git submodule add https://github.com/DocCreator/DocCreator.git augmentations
```
