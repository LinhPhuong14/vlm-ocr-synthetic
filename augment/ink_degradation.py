...
def ink_degradation(
    ...
    # The sheet: bright pixels, closed up so the odd dark glyph does not
    # punch a hole in it. Everything outside is the surface under the page.
    paper = cv2.morphologyEx(
        (grey >= config.paper_threshold).astype(np.uint8),
        cv2.MORPH_CLOSE,
