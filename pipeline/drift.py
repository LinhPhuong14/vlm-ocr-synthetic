...
A shard is checked against itself by `pipeline/invariants.py`: every image, every
...
`pipeline/invariants.paired_content` refuses to start a run where they would not.
...
def expected_shares(shard: dict, plan: dict, *, rules=None,
    ...
    root = str(Path(__file__).resolve().parent.parent / "tools")
