...
| [`layouts64/`](layouts64) | 64 | **mọi bố cục đang bật, hai lần, không hai ảnh liền kề cùng bố cục** — 32 layouts × 2, 5 530 boxes, and the first set on which the `ornament` attribute is actually printed: 48 seals, watermarks and QR codes over 33 pages. Was `layouts84` until root 3 (Form) was switched off — the count is in the name and moves with it. See its own [README](layouts64/README.md) |
