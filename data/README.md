...
| [`page_curl20/`](page_curl20) | 20 | the look before `augmentation.page_curl` went `enabled: true` — a real Blender render per page (`degradation/blender/`), cropped to the page's own footprint, 15 distinct layouts, 20/20 rendered clean, avg 96.9s/image; timing and per-image status in its own [README](page_curl20/README.md) |
| [`warp_variety20/`](warp_variety20) | 20 | five `degradation/blender/` warp scenarios, four pages each — `page_curl`, `folded`, `lifted_corner`, `crease_bundle`, `crumple` — surface deformation only, not SyntheticDoc's "dropped on an obstacle" family; 20/20 rendered clean, avg 100.4s/image, caught a real box-remapping bug along the way — see its own [README](warp_variety20/README.md) |
