...
def _render(backend: str, out: Path, args: list[str]) -> list[tuple]:
    ...
    pages = []
    # `record.read` walks the images in a directory and takes the record beside
    # each one. It used to be handed `metadata.jsonl`, which no renderer writes
    # any more -- and a missing index read as an empty dataset, so this test
