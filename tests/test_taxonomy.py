...
NEEDS_COVERAGE = (
    "rulebase.documents is not in this branch, so the tree's `status` field is "
    "NOT being checked against the rules and the builders. This is the check "
    "the field exists for; skipping it is 'we could not look', not 'it is "
    "fine'. It needs `rulebase/documents.py` and one optional `doc_type` on "
    "`spec.Option` -- both separable from the rules/document directory move "
    "that the rest of branch 800303 was rejected for."
)

try:                                   # noqa: SIM105
    from rulebase.documents import coverage as _coverage
except ImportError:                    # pragma: no cover - the rescued state
    _coverage = None


@pytest.mark.skipif(_coverage is None, reason=NEEDS_COVERAGE)
