...
Measured by `tools/profile_pipeline.py` (`make profile`), over 8 images per
renderer. Kept here rather than beside the numbers it was taken from, because
`data/` is output and this is the reasoning several design decisions rest on --
`src/worklist.py` and `src/pipeline/worker.py` both cite it. Re-run the profile
after a change to the renderers and update the tables below.
...
That was the largest lever this profile found, and it was not in a renderer or in a degradation model: it was the shape of the invocation. The renderers take a job list now (`src/worklist.py`), and the same plan went from 140 s to 98 s measured end to end. The saving predicted from this model before the change was made came within 7.3% of the saving measured after it -- which is the first time the cost model was used rather than merely built.
