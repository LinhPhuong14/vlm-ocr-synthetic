"""Tab 1: sinh ảnh, xem trực tiếp từng ảnh vừa xong trong một gallery.
...
def generate(n: float, layouts: list[str], augmentation: str, workers: float):
    ...
    `yield` to the gallery/status/state outputs instead of waiting for return."""
    n = int(n)
...
def show_detail(metadata: list[dict], evt: gr.SelectData) -> str:
    if not metadata or evt.index is None or evt.index >= len(metadata):
        return "_(chưa chọn ảnh)_"
    return live_run.detail_markdown(metadata[evt.index])
