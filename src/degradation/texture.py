...
# `src/degradation/texture.py` -> parents[2] is the repository root.
TEXTURE_ROOT = Path(__file__).resolve().parents[2] / "assets" / "textures"
...
# are used differently from `assets/textures/paper/`: a file in PAPER_DIR is the sheet
...
OVERLAY_DIR = Path(__file__).resolve().parents[2] / "assets" / "overlays"
...
def _synthetic_paper(shape: tuple[int, int], rng: random.Random) -> np.ndarray:
    """A fallback sheet, for when `assets/textures/paper/` is empty."""
    height, width = shape
