...
[`assets/textures/paper/`](../../assets/textures/paper) and is named by
`src/rulebase/rules/visual.yaml` — as one sheet, or as a shortlist to draw from.
...
| --- | --- | --- |
| [`assets/textures/paper/`](../../assets/textures/paper) | before anything is drawn | the sheet the text is printed on. `paper_texture` only ever darkens, so ink stays ink. Eight sheets, all generated: four smooth (thermal, recycled, office) and four coarse (wood, stone, weave) whose grain reads as rough stock under a 0.3–0.5 alpha. |
| [`assets/overlays/`](../../assets/overlays) | last step of the chain | photographs of real sheets, from SynthDoG's `resources/paper/`, laid over the finished page. `paper_overlay` multiplies *and* screens, so fibre darkens while the sheet's own scatter lifts the page — a render that has been through both reads as printed on paper rather than pasted onto a picture of paper. |
| [`assets/textures/background/`](../../assets/textures/background) | after ageing, glyph renderer only | the scene the sheet is photographed on, from SynthDoG's `resources/background/`. The two HTML renderers produce flat scans with no surround. `background.image.paths` in `generators/synthdog/config_vi_receipt.yaml` points at it. |
...
comes from `src/rulebase/rules/augmentation.yaml`.
