"""Every check that must pass before the first image is drawn.
    ...
    python src/pipeline/preflight.py            # or: make preflight
...
# `src/pipeline/x.py` -> parents[0]=pipeline, [1]=src, [2]=the repository.
...
# Taken from the rule-base rather than rebuilt from REPO_ROOT: a second spelling
# of the same directory is a second thing to fix when the tree moves, and it
# fails silently -- an empty glob reads as "nothing to check", not as "wrong
# path". `rulebase` is the one that knows where its own files are.
LAYOUTS_ROOT = rulebase.LAYOUTS_ROOT
...
def printable_text() -> set[str]:
    ...
    * strings the rules own rather than the corpus, which by now is most of a
    ...
    section added to `src/rulebase/layout.py` printed strings from a key the list
    ...
    for path in sorted((corpus.CORPUS_ROOT / "vi").glob("*.txt")):
...
def font_coverage(characters: set[str]) -> list[str]:
    """Report any font in `assets/fonts/` that cannot draw a character we may print.
...
ORNAMENT_ROOT = REPO_ROOT / "assets" / "textures" / "ornament"


def ornament_assets() -> list[str]:
    """Every ornament a rule names has to exist, and every file has to be named.

    `rules/ornament.yaml` refers to `assets/textures/ornament/*.png` by stem, so a
        ...
        for mark in option.params.get("marks") or []:
            ...
            if stem not in on_disk:
                problems.append(
                    ...
                    f"assets/textures/ornament/{stem}.png does not exist; run `make ornaments`")
    for stray in sorted(on_disk - named):
        problems.append(
            f"assets/textures/ornament/{stray}.png: no rule in rules/ornament.yaml names it, "
...
# per cent for every font in `assets/fonts/`, which is all this check needs -- it is
