...
# `src/pipeline/config.py` -> parents[0]=pipeline, [1]=src, [2]=the repository.
REPO_ROOT = Path(__file__).resolve().parents[2]

# Where a run-specific rules directory is announced to the renderers. Unset
# means the shipped `src/rulebase/rules/`, which is why a run without overrides is
...
class Config:
    ...
    # `src/rulebase/layouts/`", which is right for a dataset and wrong for a fixed
