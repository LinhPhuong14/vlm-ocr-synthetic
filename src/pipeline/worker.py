"""Render one shard, completely or not at all.
    ...
    python src/pipeline/worker.py --plan data/run01/plan.json --shard 3 --out data/run01
...
Since W2 a shard also checks what it drew -- `src/pipeline/invariants.py`, called
...
cost in the generator, measured in `docs/where-the-time-goes.md` and in none of the
...
def renderer_command(backend: str, staging: Path, jobs: Path,
    ...
    start-up in full. Measured in `docs/where-the-time-goes.md`, that was 23% of a
...
def render_shard(shard: dict, out: Path, plan: dict, *, rules_root: Path | None = None,
    ...
    # whole plan and happens in `src/pipeline/run.py`; what cannot wait is the
