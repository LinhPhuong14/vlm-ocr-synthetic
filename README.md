...
time either shape moves it moves in one file.

Every committed dataset is already in this shape, so nothing here needs
converting. A set generated before it and kept elsewhere is brought forward by
`pipeline.record.migrate`, which re-renders nothing: every value it writes is
already in the old record, or in the JPEG header beside it. It lives in
`record.py` beside `record.build` rather than in a tool of its own, because a
record has one definition and both callers reach it — a renderer with pixels in
hand, and the converter with an old line in hand.
