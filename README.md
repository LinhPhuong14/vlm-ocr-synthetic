...
| **merged cells** — [`rulebase/layout.py`](rulebase/layout.py) | `span:`, `column_groups:` | a cell covering several columns or rows, with no rules drawn inside it. The frame subtracts the merge from the lines it would have drawn, which is the whole of what merged means on paper |
| **table variation** — [`rules/_table_variation.yaml`](rulebase/rules/_table_variation.yaml) | `variation: ruled_form` | the tint, the stripe, the blank rows, the pen weight and whether the head has two levels, re-rolled per page from the page's own seed |

Nine of the fourteen layouts are on A4; the five till receipts are on a roll.

### Tables: merged, nested, coloured

The item table is drawn from the grid all three renderers read, so a table
defect is one defect and not three. Full grammar in
[`rulebase/README.md`](rulebase/README.md#merged-nested-coloured):

| | what it is |
| --- | --- |
| **merged** | a cell covering several columns or rows has no rules inside it; the rectangle is written to the label as `table.merges`, because once those lines are absent no reader of the pixels can recover it |
| **nested** | `column_groups:` sets one title over several columns with theirs beneath — a merge across for the parent, a merge down for every column without one |
...
Two defects this fixed, both on any water bill generated before it: the
...
| `table` | only where the layout has one: which rectangles of it are a single merged cell, and which variant of the ruling this page drew |
