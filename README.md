...
[![Handwriting](https://img.shields.io/badge/Chữ_viết_tay-WriteViT-9B59B6.svg)](docs/writevit.md)
...
| **WriteViT** — [`tools/writevit/`](tools/writevit), [`docs/writevit.md`](docs/writevit.md) | Sinh **nét chữ viết tay thật** để điền vào ô trống của biểu mẫu — không phải font in bị rung. | *Mở rộng (Handwriting)* |
...
flowchart TD
    ...
    subgraph S2 ["Giai đoạn 4: Dựng pixel — generators/html/"]
        ...
        D2 --> D3["4c · điền tay (tuỳ chọn)<br/>handwriting.py + WriteViT"]
    end
...
### 7.5 Chữ viết tay thật, không phải font bị rung

![Ô trống được điền bằng nét WriteViT](docs/figures/handwriting-html.jpg)
...
│   ├── record.py                   # schema bản ghi của bộ chuyển đổi
...
| [`data/hand12/`](data/hand12) | 12 tờ mẫu **điền tay** bằng nét WriteViT |
...
| | |
| :--- | :--- |
| **`tests/test_worklist.py` gọi API cũ** | 4 test render thật gọi `record.read(out / "metadata.jsonl")`, trong khi schema đã chuyển sang **một JSON cho mỗi ảnh** và `record.read` nay nhận một **thư mục**. Hệ quả: hàm đọc về danh sách rỗng và cả 4 test đỏ. Chúng gắn `@pytest.mark.slow` và tự bỏ qua khi chưa dựng venv, nên CI (vốn không dựng venv nào) không thấy. **Đã sửa** trong commit này — một tham số — và cả 16 test trong file đó xanh trở lại. |
