...
| --- | --- | --- |
| `src/rulebase/` | any Python 3.9+ with `PyYAML` | pure content logic; no image libraries |
| `src/degradation/` | any Python 3.9+ with `numpy`, `opencv` | shared by all three renderers |
...
* `assets/fonts/` — all redistributable, and a clone cannot render without them
* `assets/textures/paper/` — generated by `make textures`, so redistributable
* `assets/textures/background/` and `assets/overlays/` — SynthDoG's
  photographs of scenes and of paper; a clone cannot produce a photographed
  receipt without them
* `samples/` and `docs/figures/` — curated examples and the README's figures;
  the point of these is that they can be looked at without running anything
* `tests/fixtures/` — the few real renders the suite cannot synthesise

Not committed: **datasets**. `data/` is output — a dataset is a pure function of
the rule-base and the renderers, so it is rebuilt with `make dataset` rather
than carried in git. Also not committed: anything a renderer writes to
`outputs/`, virtualenvs and model checkpoints.
