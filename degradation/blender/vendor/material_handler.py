# Adapted from tanguymagne/SyntheticDoc (generation/rendering/material_handler.py),
# MIT License. See ../LICENSE. `createPBRMaterial`'s "Mapping" node lookup carries a guard
# not in the original (see its own comment) -- everything else is unchanged.
...
def createPBRMaterial(material_name, texture_dir):
    ...
    # except with a single, generically-named image (no PBR suffix Node Wrangler
    # recognises), where it wires the texture straight into the BSDF and skips the
    # Mapping/Texture Coordinate nodes entirely. Guarded here rather than trusted, since
    # this repo's own textures/background/*.jpg are single generic photos, not full PBR
    # sets: a real PBR set (base colour + roughness + normal, named per Node Wrangler's
    # convention) still gets the Mapping node upstream expects.
    if "Mapping" in nodes:
