# Adapted from tanguymagne/SyntheticDoc (generation/rendering/blender_utils.py), MIT License.
# See ../LICENSE. `setShading`'s auto-smooth call carries a Blender < 4.1 compatibility
# shim not in the original (see its own comment) -- everything else is unchanged.
    ...
    if smooth:
        ...
        if auto_smooth:
            ...
            try:
                ...
                # Blender < 4.1 shim: `bpy.ops.object.shade_smooth_by_angle` does not exist
                # before 4.1 (e.g. Ubuntu's `apt install blender`, still on 4.0 at the time of
                # writing), and a missing bpy.ops operator only raises at CALL time, never on
                # attribute access -- `hasattr` cannot detect this, has to be try/except.
                # Pre-4.1 auto-smooth is a mesh property instead of a modifier operator.
